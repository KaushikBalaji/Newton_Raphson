...
module NR_stage_pipeline #(parameter N = 8, parameter W = 16)(
    ...
    // Stage 1: x^2 (QN.N * QN.N = Q2N.2N -> QN.N after truncation)
    reg [W-1:0] x_reg1, a_reg1, x_sq_reg1;
    wire [(2*W)-1:0] x_sq_full = x_in * x_in;
    wire [W-1:0] x_sq_scaled = (x_sq_full + (1 << (N-1))) >> N; // truncate at x^2 and final multiplication stage to maintain accuracy
    ...
    // wire [W-1:0] term_full = (3 << N) - ((ax2_full + (1 << (N-1))) >> N);        // truncation everywhere made converging for large number slower
    ...
    wire [W-1:0] mult_scaled = (mult_full + (1 << (N))) >> (N + 1);
